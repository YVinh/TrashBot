...
def test_generate_hashtags_first_four_include_location_tags():
    ...
    assert "#Brussels" in first_four
    assert any(tag in first_four for tag in ("#ClimateAction", "#TrashAlert"))


def test_generate_hashtags_unknown_location_falls_back_to_universal():
    tags = generate_hashtags("Nowhereville")
    assert tags[:4] == ["#ClimateAction", "#TrashAlert", "#UrbanCleanliness", "#ActOnClimate"]
...
def test_format_hashtags_adds_missing_hash_prefix():
    assert format_hashtags(["Trash", "#ClimateAction"]) == "#Trash #ClimateAction"
